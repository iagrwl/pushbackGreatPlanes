#include "lemlib/chassis/chassis.hpp"
#include "main.h"
#include "pros/distance.hpp"
#include "pros/llemu.hpp"
#include "setup.hpp"
#include "twoGoal.hpp"

void two_goal_RIGHT(){
    //nothing here


};

void two_goal_LEFT() {
    // sets position to top left of park zone facing forwards
    chassis.setPose(-6.75, -47, 0);

    // keeps the scoring bar closed
    scoringGate.set_value(true);
    // starts intake system
    frontIntake.move(127);
    middleRollers.move(127);
    scoringRoller.move(100);
    
    // keeps the wing down
    wingMech.set_value(true);

    // moves to 3 cluster and loader down after 600 ms
    chassis.moveToPose(-24, -24, -45, 3000, {.minSpeed = 40});
    //pros::delay(900);
    //loaderMech.set_value(true);
    
    //chassis.turnToPoint(-40, -10, 700);
    chassis.moveToPose(-40,-10, -90, 2500, {.lead = 0.3, .minSpeed = 60});
    //pros::delay(200);
    loaderMech.set_value(false);
    pros::delay(850);
    loaderMech.set_value(true);
    pros::delay(200);
    chassis.moveToPoint(-30, -28, 1500, {.forwards = false, .minSpeed = 40, .earlyExitRange = 3});
    chassis.swingToHeading(140, lemlib::DriveSide::LEFT, 750);
    chassis.moveToPose(-48, -22, 180, 1500, {.forwards = false, .lead = 0.1, .minSpeed = 100});
    chassis.swingToHeading(-180, lemlib::DriveSide::LEFT, 750, {.minSpeed = 100, .earlyExitRange = 20});
    chassis.moveToPoint(-48, -18, 1500, {.forwards = false, .minSpeed = 100});
    scoringGate.set_value(false);
    pros::delay(600);
    scoringGate.set_value(true);
    chassis.moveToPoint(-46, -68, 1800, {.maxSpeed = 60});
    //pros::delay(500);
    chassis.moveToPoint(-46, -48, 1000, {.forwards = false, .minSpeed = 100, .earlyExitRange = 4});
    chassis.moveToPose(-8, -8, -135, 2500, {.forwards = false, .minSpeed = 40}, false);

    frontIntake.move(-100);
    middleRollers.move(-127);
    scoringRoller.move(-127);

    pros::delay(100);

    chassis.moveToPoint(0,0,1000,{.forwards=false,.maxSpeed=20});
    colorsortOn=false;
    //scoreMidGoal();
    frontIntake.move(127);
    middleRollers.move(80);
    scoringRoller.move(-70);
    pros::delay(1300);
    middleRollers.move(127);
    scoringRoller.move(127);
    chassis.moveToPoint(-27, -32, 1500, {});
    chassis.turnToHeading(180, 750, {.minSpeed = 20, .earlyExitRange = 40}, false);

    chassis.moveToPose(-37.5, -10, 180, 3000, {.forwards = false, .lead = 0.3, .minSpeed = 30});
    wingMech.set_value(false);
}